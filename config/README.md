...
This directory contains configuration used by the top-level Hive helper scripts. It is not the same as the Go binary config in `src/hive.yaml.example`.
...
| `src/hive.yaml.example` | The Go `hive` binary, dashboard, governor, agent manager, hub, and API server | Runtime config for the containerized Hive service: project, agents, governor modes, GitHub auth, dashboard auth, data paths, knowledge, hub/spoke settings, and ACMM level. |

Use `src/hive.yaml` for normal Docker/Kubernetes operation. Use `hive-project.yaml` only when you run the top-level `bin/` scripts or the deterministic pipeline directly.
