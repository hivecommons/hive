    ...
    if [[ -z "$TOKEN" || "$TOKEN" == "null" ]]; then
      if echo "$MSG" | grep -qi "already registered"; then
        if [[ -f "{{config_dir}}/contributor.env" ]]; then
          ...
          echo ""
          echo "  1. KEEP the credential — move it here, encrypted (#8127):"
          echo "       (on the old machine)  hivectl hives export <name>"
          echo "       (copy <name>.hiveprofile across, then here)"
          echo "       hivectl hives import <name>.hiveprofile"
          echo "     The bundle is passphrase-encrypted, so the token is never at rest"
          echo "     in the clear the way an scp'd contributor.env is; gh-auth.env was"
          echo "     already written by step 1 above. The hub stores only a HASH of the"
          echo "     registration token and clears the plaintext after first read, so"
          echo "     moving the credential is the ONLY way to reuse it — it can never"
          echo "     be printed again. Both machines then authenticate as the SAME"
          echo "     contributor: run one relay per identity (or label each with"
          echo "     'hivectl hives session'), and remove the profile on the machine"
          echo "     you are moving off."
          echo ""
