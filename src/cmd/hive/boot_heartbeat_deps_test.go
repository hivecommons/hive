...
type bootHeartbeatFake struct {
	...
	collect         hub.StatusCollector
	callbacks       []any
	taskURL         string
	taskCollect     hub.TaskStatusCollector
	heartbeatStarts int
}
...
func newBootHeartbeatFake() *bootHeartbeatFake {
	...
	f.deps = bootHeartbeatDeps{
		...
		startHeartbeat: func(_ context.Context, hubURL string, collect hub.StatusCollector, interval time.Duration, _ *slog.Logger, callbacks ...any) {
			f.heartbeatStarts++
			f.heartbeatURL, f.collect, f.interval, f.callbacks = hubURL, collect, interval, callbacks
		},
		startTaskStatusPush: func(_ context.Context, hubURL string, collect hub.TaskStatusCollector, _ *slog.Logger) {
...
func TestBootHeartbeatWith_PublishesIdentityBeforeTheLoopAndWiresEveryCallback(t *testing.T) {
	...
	heartbeatCallback[hub.RestartSpokeCallback](t, f)
	heartbeatCallback[hub.UpgradeCallback](t, f)
	heartbeatCallback[hub.GitHubAppConfigCallback](t, f)
	heartbeatCallback[hub.HubBannerCallback](t, f)
	heartbeatCallback[hub.UpgradePolicyCallback](t, f)
	heartbeatCallback[hub.VisibilityCallback](t, f)
	heartbeatCallback[hub.SwitchBranchCallback](t, f)
	heartbeatCallback[hub.AgentRestartResetCallback](t, f)
	heartbeatCallback[hub.AuthorizedUsersCallback](t, f)
	heartbeatCallback[hub.ProjectConfigCallback](t, f)
	heartbeatCallback[hub.GatewayConfigCallback](t, f)
	if len(f.callbacks) != 11 {
		t.Fatalf("registered %d callbacks, want 11 (one of each)", len(f.callbacks))
	}
...
func TestBootHeartbeatWith_CallbacksReconcileRunningConfig(t *testing.T) {
	...
	heartbeatCallback[hub.VisibilityCallback](t, f)(true)
	...
	users := heartbeatCallback[hub.AuthorizedUsersCallback](t, f)
	...
	banner := heartbeatCallback[hub.HubBannerCallback](t, f)
	banner(&hub.HubBanner{ID: "b1", Message: "maintenance", Color: "amber"})
	banner(nil)
	heartbeatCallback[hub.UpgradePolicyCallback](t, f)(&hub.HeartbeatUpgradePolicy{HubManaged: true})
	heartbeatCallback[hub.UpgradePolicyCallback](t, f)(nil)

	// Unknown agent: warn, never panic — the hub may name an agent this
	// spoke's pack no longer has.
	f.log.Reset()
	heartbeatCallback[hub.AgentRestartResetCallback](t, f)("ghost")
	...
	f.log.Reset()
	gw := heartbeatCallback[hub.GatewayConfigCallback](t, f)
	gw(nil)
	gw(&hub.HeartbeatGatewayConfig{Name: "openrouter"})
