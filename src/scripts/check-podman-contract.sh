...
# src/docs/podman-rootless-ci.md for what this covers and what a future live
# test would add. It does NOT touch the runtime auto-detect default (that's
# Option A, explicitly deferred).
#
# Usage: src/scripts/check-podman-contract.sh [path-to-Justfile]
set -euo pipefail
...
# alongside src/docs/podman-rootless-ci.md, don't just delete it.
check "auto-detect still tries docker before podman" \
  'command -v docker.*RUNTIME=docker'

if [[ "$fail" -ne 0 ]]; then
  echo ""
  echo "Rootless Podman contract check FAILED — see src/docs/podman-rootless-ci.md"
  exit 1
fi
