...
#
# Invariant (asserted against src/Dockerfile):
...
#
# Usage: src/scripts/check-suid-contract.sh [path-to-Dockerfile] [path-to-entrypoint]
set -euo pipefail

DOCKERFILE="${1:-src/Dockerfile}"
ENTRYPOINT="${2:-src/deploy/entrypoint.sh}"
