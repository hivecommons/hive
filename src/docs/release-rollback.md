...
Scope note: this page is written for the release train, not only for the
current `v4` branch. The first-class hosted-spoke digest-pin API described
below is implemented on the `v5` branch; `v4` hosted hives do not have
`/api/saas/hives/{id}/pin-digest`, so their digest rollback path is the direct
Deployment patch plus the upgrade pause called out below. Self-managed
Kubernetes, Compose, and Quadlet pins work on `v4` today because they edit the
runtime image reference directly.
...
|---|---|---|
| `ghcr.io/hivecommons/<image>:<7-hex-sha>` | `docker.yml`, every successful build of a release line | A bounded rollback/debug window, not a permanent release. The kubestellar pruning workflow deletes package versions whose only tags are short-SHA tags after **90 days**; the hivecommons twin has the same `RETENTION_DAYS` rule but is dry-run by default until maintainers deliberately enable deletion. Versions still carrying any moving tag are never pruned. |
...
## Switching channels or release lines

A channel switch deliberately writes a **moving tag**, not a digest. Use it
when you want the hive to keep following a track; use the rollback steps below
when you want one known-good build to hold still.

For a hosted spoke, the hub endpoint is the same branch-switch endpoint the
dashboard uses. The target goes in the `branch` field verbatim:
...
Valid channel targets are `stable`, `candidate`, and `edge`. A branch target
is converted to `<branch>-latest`, so `{"branch":"v5"}` selects
`ghcr.io/hivecommons/hive:v5-latest`; a channel target is already the tag, so
`{"branch":"edge"}` selects `ghcr.io/hivecommons/hive:edge`. The switch
handler validates that the tag exists on GHCR before it writes the Deployment,
then either patches `deployment/hive` in namespace `hive-hosted-<hive-id>` or
queues the tag for the spoke's next heartbeat if the hub cannot reach the
cluster.

For self-managed deployments, make the same choice in your source of truth and
apply it:

```bash
# Kubernetes or kustomize overlay: track a line, track a channel, or pin a digest.
kubectl -n <namespace> set image deployment/hive \
  "*=ghcr.io/hivecommons/hive:v5-latest"
kubectl -n <namespace> rollout status deployment/hive

# Compose: edit services.hive.image, then recreate the service.
docker compose up -d hive

# Quadlet: ask the update helper to resolve and pin the selected tag's digest.
bin/hive-podman-update.sh pin ghcr.io/hivecommons/hive:edge
```

The published channel meaning is line-specific:

| Target | What it follows |
|---|---|
| `stable` | The v4 stable promotion result, moved by `.github/workflows/promote-stable.yml` after the soak gate. |
| `candidate` | The newest publishing `v4` build selected by `.github/workflows/docker.yml`. |
| `edge` | The active-development v5 channel on the `v5` branch. |
| `v4-latest` / `v5-latest` | The latest publishing build on that branch. |
| `vX.Y.Z` | An immutable tagged release image, currently cut by the v4 tagged-release workflow only. |

Do not deploy `latest` as a release-line selector. The workflow arguments
decide per image and per branch whether the global `latest` tag is written;
the line/channel/digest names above are the operationally meaningful refs.
...
A pin taken through the v5 hub API (step 3) is a **run-state on the hive record**,
...
On `v4`, that API does not exist. Use the admin **upgrade pause** before a
manual hosted-spoke Deployment patch, because the durable channel re-arm can
otherwise put a channel-tracking hive back on its channel at the next
heartbeat. On `v5`, the pause is no longer a prerequisite for rolling back
one hive through the pin API. It remains the right tool when the incident is
fleet-wide and you want *every* image change stopped while you decide what to
pin:
...
**Hosted spoke** (`hive`), through the v5 hub. This is the sanctioned v5 path:
it needs hub owner (or admin) rights and no cluster `kubectl` access, it
writes the same `deployment/hive` object with the same `*=` form the hub's
upgrade path uses, and it records the pin on the hive:
...
On `v4`, or on a v5 cluster the hub cannot reach, the direct patch remains
available from a kube-context that reaches the spoke's cluster:
...
On `v4`, the hub has no record of that pin and its channel re-arm can undo it
on the next heartbeat, so pair the patch with the fleet-wide upgrade pause
from step 2 and leave the pause on for as long as the rollback must hold. On
`v5`, prefer the API path when the hub can reach the cluster; if it cannot,
the API refuses rather than recording a pin it cannot deliver.
...
For a v5 hosted-spoke pin, also confirm from the hub's side, which sees the
spoke only through its heartbeat:
...
`--version` string. The binary's semver field is currently `0.0.0-dev` even
for tagged-release images, because the release workflow retags an ordinary
`docker.yml` image rather than rebuilding with a `VERSION` build arg (see
"The version constant" in [releases.md](releases.md)); only the git hash and
the digest identify a build.

## Step 5: leave it pinned, or return to a moving tag

A digest-pinned hive is deliberately outside the upgrade train. On v5 the hub
reports an API pin as pinned, never auto-upgrades it, and never counts it as
behind. On v4 a direct Deployment patch is not a hub run-state, so keep the
upgrade pause or your GitOps pin in place for as long as the rollback needs to
hold.

To rejoin a moving tag later from a v5 API pin, lift the pin:
...
For direct pins, replace the digest with the desired channel, branch, or
version tag in the Deployment/Compose/Quadlet source of truth and roll the
workload again. Switching is considered complete when the heartbeat's reported
