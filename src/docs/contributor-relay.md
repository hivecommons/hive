...
- **The `unprovisioned` admit is NOT flipped to a hold here (kubestellar/hive#6987), and the divergence is recorded on purpose.** #6987's title paired "make publishing default-on" with "flip `unprovisioned` to a hold"; only the first ships in this change, and the second is deliberately left out. The reason is the one #6951's ruling turned on: making publishing default-on is the prerequisite that gives a default install a *route* to a reading, but the flip must not ride ahead of a guarantee that every supported host actually has a publisher running — rotation is itself opt-in, and #6986 showed an adapter can silently report permanent `unknown`. So the two dirs behave differently, on purpose:
  ...
  - With the **derived default** dir (no env var), a *present* reading is evaluated normally (a torn or `state: unknown` file still **holds** — fail-closed), but a **missing** reading falls through to `unprovisioned`/admit. A default install with no publisher running must never be stranded holding forever; that is the exact fleet-wide stop #6951's `unprovisioned` admit was created to avoid. Flipping the default `unprovisioned` to a hold stays a one-line policy change for the maintainer to take deliberately, and the precondition is now concrete and checkable rather than a matter of judgement: **publishing is wired only inside the `if w.cfg.Governor.Rotation.Enabled` branch (`src/cmd/hive/worksourcewire.go`), and `RotationConfig.Enabled` is `false` by default (`src/pkg/config/governor_config.go` — "Default false — opt-in").** So a supported backend does *not* imply a live publisher: every install that has not opted into provider rotation would be stranded on a permanent hold the moment the flip lands. The flip becomes safe when either (a) the publisher no longer depends on `governor.rotation.enabled` — it runs for any supported backend — or (b) the guard can positively distinguish "a publisher is expected here but has not written yet" from "nothing will ever write here", which today it cannot, because both cases present identically as a missing file.
...
- **With no reading source at all, the guard is inert and says so.** The guard
  ...
  live publisher — which is **not** yet the case: publishing runs only when
  `governor.rotation.enabled` is true, and that defaults to false, so the flip
  would today strand every install that has not opted into provider rotation.
  See "How the reading reaches the guard" for the two concrete conditions that
  would make it safe (kubestellar/hive#6987 landed the default-on publishing
  that makes that flip safe to *consider*; it does not take the flip itself).
