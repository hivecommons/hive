...
shared rule table; hovering a chart element, an Overview legend row, or a
repository-card band header shows that band's rule from `ACTION_BANDS` /
`actionBandRule` in `index.html`, so the three cannot drift.
