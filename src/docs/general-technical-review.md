...
- **`NOTICE` data not yet authoritative** — the attribution mechanism now
  exists (generator, `notice-drift` CI guard, release attachment), but the
  committed `NOTICE` is a statically-derived placeholder whose license
  fields all read `UNVERIFIED`. No license was inferred. The generator could
  not run at all until [#5010](https://github.com/kubestellar/hive/pull/5010)
  (a repo-layout defect: `LICENSE` at the root, module in `src/`); after that
  lands, committing the real output is tracked in
  [#5007](https://github.com/kubestellar/hive/issues/5007)
  ([attribution](#what-steps-does-the-project-take-to-ensure-that-all-third-party-code-and-components-have-correct-and-complete-attribution-and-license-notices)).
