...
- Recovery from a provider spending limit is automatic, via a probe. Withholding kicks also withholds the inference calls that would reveal the provider is serving again, so the hive suppresses only while the last refusal is recent and then lets one cycle's kicks through to test the gateway. A still-clipped key refuses the probe and suppression resumes for another interval; once the provider's window resets the probe succeeds and normal kicking resumes with no operator action. Tune with `governor.provider_budget.probe_interval_s` (default 1800 — 30 minutes):
