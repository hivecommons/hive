...
| `HIVE_AGENT_ROLE` | No | none | Optional spoke agent role the relay claims at registration (scanner, quality, outreach, …; hub-enforced). |
| `HIVE_AGENT_SESSION` | No | `contributor` | tmux session name the relay drives (interactive mode). |
| `HIVE_AGENT_CWD` | No | relay's own cwd | Neutral directory (`$HOME`) both contributor entrypoints export so a CLI relaunch is pinned to the directory the pane was started from, not wherever the relay happens to be running. |
| `HIVE_CONTRIBUTOR_USERNAME` | No | `unknown` (set by `contributor-agent.sh`) | The GitHub login this contributor pushes and opens PRs as; used to decide whether a PR seen in the pane is this relay's own ([#6662](https://github.com/hivecommons/hive/issues/6662)). The literal `unknown` means "identity not available" and skips the authorship check rather than refusing every PR. |
| `HIVE_GH_TOKEN_CACHE` | No | `/var/run/hive-metrics/contributor-gh-token.cache` if that directory exists, else `/tmp/hive-gh-token.cache` | Where the hub-delivered, task-scoped GitHub token is written. Deliberately **not** `gh-app-token.cache` — that filename is the hub's full-privilege installation-token cache, which a relay co-located with hub components must never clobber or misreport as its own credential ([#3842](https://github.com/hivecommons/hive/issues/3842) audit N14). |
| `HIVE_TASK_FILE` | No | `/tmp/contributor-task.json` | Where the relay records the currently assigned task as JSON for the agent and supervising tooling to read. |
| `HIVE_WORKSPACE_DIR` | No | relay's own cwd | Workspace the headless one-shot CLI runs in, exported by `contributor-agent.sh` and pinned at relay startup so a later environment mutation cannot move the child outside the directory granted to it (e.g. Codex `--add-dir`). |
| `HIVE_ABSOLUTE_TASK_DEADLINE_MS` | No | `14400000` (4 h) | Ceiling on total elapsed time from task assignment that nothing re-arms — the backstop for a task that produces output forever (a retry loop redrawing a spinner is output) and would otherwise hold its progress lease indefinitely. Crossing it is reported as an `environment` failure. See "Task ceilings" in [contributor-relay.md](contributor-relay.md). |
| `HIVE_HEADLESS_TASK_TIMEOUT_MS` | No | `HIVE_ABSOLUTE_TASK_DEADLINE_MS` | Hard ceiling on a single headless one-shot CLI invocation ([#2538](https://github.com/hivecommons/hive/issues/2538)). The headless path has no pane to scrape for progress, so the absolute bound is enforced directly on the child process; per [#5321](https://github.com/hivecommons/hive/issues/5321) a long-but-live headless run is no longer killed at 30 minutes. |
| `HIVE_PANE_STALL_TIMEOUT_MS` | No | `1200000` (20 min) | How long the pane may stay byte-identical while the relay reports "working" before it stops asserting progress it cannot substantiate and hands the task back as an `environment` failure. Deliberately generous: a real agent can sit on one silent command (a long test suite, a slow clone) for many minutes. |
| `HIVE_PANE_STALL_CONFIRM_TICKS` | No | `2` (min 1) | Consecutive progress ticks (120 s apart) that must each confirm the stall before the relay gives up, giving a CLI blocked on one slow network call a few more ticks to reach a real completion with a genuine PR link. Any new output or an idle-complete pane resets the count. |
| `HIVE_CHROME_IDLE_GRACE_TICKS` | No | `3` (min 1) | Consecutive idle-complete ticks with no completion verdict before the relay accepts an unverdicted completion, marked `chrome_idle` so the hub can tell which signal ended the task ([#5376](https://github.com/hivecommons/hive/issues/5376)). |
| `HIVE_HUMAN_PRESENCE_IDLE_MS` | No | `300000` (5 min) | How recently a tmux client must have been active for the relay to treat the session as having a human at it and suppress automatic "try again" recovery typing. Below ~2 minutes the threshold is unobservable (the check runs on the 120 s progress tick). |
| `HIVE_TMUX_COMMAND_TIMEOUT_MS` | No | `15000` | Timeout on each tmux command the relay executes (send-keys, capture-pane, respawn-pane). |
| `HIVE_LIVE_CLI_FIRST_INTERRUPT_DELAY_MS` | No | `1000` | Pause after the first `C-c` when terminating a live CLI, before the second is sent ([#2203](https://github.com/hivecommons/hive/issues/2203) wedge shape). |
| `HIVE_LIVE_CLI_SECOND_INTERRUPT_DELAY_MS` | No | `2000` | Pause after the second `C-c` before checking whether the pane fell back to a shell. |
| `HIVE_LIVE_CLI_SHELL_WAIT_TIMEOUT_MS` | No | `5000` | How long to wait for the pane to become a shell after interrupts; a backend that absorbed both `C-c`s (pi, omp — [#7733](https://github.com/hivecommons/hive/issues/7733)) is then forced down with `tmux respawn-pane -k` instead of having the relaunch line typed into its live TUI as a chat prompt. |
| `HIVE_LIVE_CLI_SHELL_WAIT_POLL_MS` | No | `250` | Poll interval for the shell-wait check above. |
| `HIVE_LIVE_CLI_RESPAWN_SETTLE_MS` | No | `500` | Settle pause after `tmux respawn-pane -k` before re-checking for a shell. |
| `HIVE_CLAUDE_PROJECTS_DIR` | No | `$HOME/.claude/projects` | Session-transcript location the relay tails (last 64 KiB) to auto-detect the running model when `AGENT_MODEL` is unset ([#4117](https://github.com/hivecommons/hive/issues/4117)); only the single model field is extracted, never logged or transmitted content. |
| `HIVE_COPILOT_SESSIONS_DIR` | No | `$HOME/.copilot/session-state` | Copilot equivalent of `HIVE_CLAUDE_PROJECTS_DIR` for model auto-detection. |
| `HIVE_BOB_DIR` | No | `$HOME/.bob` | Bob equivalent of `HIVE_CLAUDE_PROJECTS_DIR` for model auto-detection. |
