...
| `OPENAI_HOST` | No | Goose CLI default (`api.openai.com`) | OpenAI-compatible endpoint host consulted by Goose. Forwarded into the contributor container when set on the host, so Goose can reach an OpenAI-compatible local inference server instead of falling back to `api.openai.com`. |
| `OPENAI_BASE_PATH` | No | Goose CLI default (`v1/chat/completions`) | OpenAI-compatible request path appended to `OPENAI_HOST` by Goose. Forwarded into the contributor container when set on the host. |
