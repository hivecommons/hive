...
- [Standalone Hive under Podman: the service Quadlet unit](podman-standalone-quadlet.md) — the `.container` and `.volume` units that start Hive on Podman in both root modes, the install and boot-persistence steps for each, and why `systemctl start` returning means the healthcheck passed rather than merely that a process was spawned.
