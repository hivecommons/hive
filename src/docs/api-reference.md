...
| `POST` | `/api/effort/{agent}/{effort}` | Owner only | Reasoning Effort Set — validated against the agent's live backend, then the session restarts (the effort exists only on the launch command line); `{effort}` = `default` clears it | `pkg/dashboard/api.go:95` |
