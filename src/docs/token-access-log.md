...
dev:node` and **not agent-writable** (#4044 — the same directory holds the
bot-identity file the gh-wrapper author gate trusts). Agents therefore cannot
create the file; the container entrypoint pre-creates it as `dev:node 0664`
(#6106) so that only this one file in the directory accepts agent appends.
...
- `/var/run` is tmpfs-backed in the standard deployment: the log does not
  survive a container restart. Treat it as a recent-activity window, not an
  archive. The durable accountability record is [`/data/audit.jsonl`](audit-log.md).
