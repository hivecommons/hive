...
| Formal-model safety gate | Formal-verification expectations for v5 are documented and the escalation/merge/hold models have no expected-fail safety witnesses for the GA candidate. | The formal-verification workflow/test results plus any model witness tracker referenced from `src/docs/formal-verification.md`. | **Evidence-ready:** the `v5` verification matrix (`src/formal/escalation/run.sh`) carries 12 rows, all expecting `pass` — zero expected-fail safety witnesses remain in GA scope — and the latest Formal Verify runs are green (2026-09-09T17:45Z, [run 34384844388](https://github.com/hivecommons/hive/actions/runs/34384844388)); measurement recorded in [#6890](https://github.com/hivecommons/hive/issues/6890). The workflow is path-triggered on the modeled code, so no unverified drift exists. Row closes when a maintainer records the evidence on [#6016](https://github.com/hivecommons/hive/issues/6016) and re-verifies at GA cut. |
| Hold and merge-lane hygiene | Hold-guard, merge-eligible, and self-merge lanes reject moved heads and have regression tests covering the v5 branch-hygiene incident class. | `go test` results for the relevant packages (`cmd/hive`, holdguard, scheduler) and linked issue/PR evidence such as #5589. | **Measurable, evidence source red on infra:** v2 Tests (the `go test` source for these packages) has failed on every `v5` push since 2026-09-13 at the cgo-toolchain step — the [#6870](https://github.com/hivecommons/hive/issues/6870)/[#6648](https://github.com/hivecommons/hive/issues/6648) apt-mirror egress loss, not a code regression. Last `v5` SHA with green go-tests: `2b95e1a88` (2026-09-12T06:54Z, [run 34679328916](https://github.com/hivecommons/hive/actions/runs/34679328916)); measurement recorded in [#6890](https://github.com/hivecommons/hive/issues/6890). Any GA candidate after that SHA cannot currently evidence this row until #6870 is resolved. |
