...
- **Hosted fleet:** the hub sweeps hosted spoke Deployments every 15 minutes and patches `NET_ADMIN` into the container capability list where missing. It preserves the rest of the security context, including `RuntimeDefault` seccomp and capability drops. (On OpenShift/OVN the podspec request is necessary but not sufficient; the SCC must also allow it.)
...
- `su-exec` is mode **4750 `root:hive-launch`** — only root and members of the pinned `hive-launch` group (GID 1002) can exec it. This closes the earlier world-executable-setuid hole where any agent UID could become root in the pod. The Kubernetes container drops `SETUID` and `SETGID` from its capability bounding set; the helper's set-id mode remains the narrowly group-gated UID-switch mechanism. `HIVE_LAUNCH_GID=1002` is part of the deployment contract: Kubernetes `fsGroup` and Secret `defaultMode: 0440` rely on the numeric GID, and it must stay in sync across `src/deploy/k8s/*.yaml` and hub provisioning.
