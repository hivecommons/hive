...
(`src/pkg/agent/manager.go:2228`, `newSessionCommands`, which issues a plain
`tmux new-session -d` at `src/pkg/agent/manager.go:2244`) and then *types the CLI
invocation into that shell*:

```go
m.tmuxSendLiteralForAgent(agent, fullCmd)   // src/pkg/agent/manager.go:3022
time.Sleep(textToEnterDelay)
m.tmuxSendEntersForAgent(agent)             // src/pkg/agent/manager.go:3024
```

The composed command line is per-backend and is built as a string —
`claude --model … --dangerously-skip-permissions`
(`src/pkg/agent/manager.go:10671`), `copilot … --allow-all`
(`src/pkg/agent/manager.go:10698`), and so on.

Nothing in the kick path ever spawns a process. `deliverKickLocked`
(`src/pkg/agent/manager.go:5176`) only manipulates the existing pane.

### 1.2 A turn starts with simulated keystrokes

`SendKick` (`src/pkg/agent/manager.go:5083`) is the entry point; it delegates to
`deliverKickLocked` (`src/pkg/agent/manager.go:5176`), which:

- clears any stale input with `C-c` then `C-u`
  (`src/pkg/agent/manager.go:5194`),
- types the prompt literally, chunked, via
  `tmuxSendLiteralForAgent` (`src/pkg/agent/manager.go:5222`, implementation at
  `src/pkg/agent/manager.go:5513` → `tmux send-keys -l`),
- submits with `tmuxSendEntersForAgent` (`src/pkg/agent/manager.go:5236`).
...
`waitForInputPromptForAgent` (`src/pkg/agent/manager.go:4394`) polls
`captureTmuxPaneForAgent` and returns once `paneShowsInputPrompt`
(`src/pkg/agent/manager.go:4321`) matches a per-backend marker string. `SendKick`
calls it at `src/pkg/agent/manager.go:5152`. This gate is the only thing that
...
(`src/pkg/agent/manager.go:3235`) runs a `3 * time.Second` ticker
(`src/pkg/agent/manager.go:3236`) for the agent's whole lifetime, diffing
captured pane content to maintain `agent.LastPaneChange`
(`src/pkg/agent/manager.go:3311`) — hive's only evidence that a running,
authenticated CLI is actually doing something.

Stall recovery is likewise textual: `nudgeIfKickStalled`
(`src/pkg/agent/manager.go:6245`) compares `paneContentHash(pane)` against
`agent.lastInferKickPane` (`src/pkg/agent/manager.go:6273`), and
`paneShowsActiveWork` (`src/pkg/agent/manager.go:6093`) matches a spinner or a
...
| Agent-name → UID allocation | `/var/run/hive/uid-map.json` | `UIDMapPath` `src/pkg/agent/uidmap.go:17`; load `src/pkg/agent/manager.go:2114` |
| Backend CLI's own session/credential files | the CLI's own `HOME` / `CODEX_HOME`, rooted at `/data/home` | per-agent `CODEX_HOME` `src/pkg/agent/manager.go:9413`, helper `:7254`; per-agent HOME `src/pkg/agent/interactive_home.go:57`; shared `.claude` bridged by symlink `interactive_home.go:74` |
...
- The UID map is the one entry **not** rooted at the `/data` PVC. `/var/run` is
  ...
  (`src/pkg/agent/manager.go:2114-2118`), so this is a durability *asymmetry* rather than
...
|---|---|---|
| Live output ring buffer | `OutputBuffer *RingBuffer` — pure in-memory circular slice, no persistence path | `src/pkg/agent/manager.go:253`; `src/pkg/agent/ringbuffer.go` |
| Last captured pane content | `lastPaneCapture []string`, guarded by `paneMu` | `src/pkg/agent/manager.go:254-255` |
| Activity clock | `LastPaneChange` (guarded by `paneMu`) | `src/pkg/agent/manager.go:320` |
| Most recent kick text | `LastKickMessage` — replaced on every kick; the durable copy is truncated (`KickRecord.Snippet`) or lives in prompt-history | `src/pkg/agent/manager.go:256`; explicitly called out at `src/pkg/dashboard/prompt_history.go:26-31` |
| tmux session/socket identity | `tmuxSession`, `tmuxSocket` | `src/pkg/agent/manager.go:262-263` |
| Per-launch cancel func and launch generation | `cancel context.CancelFunc`, `launchGen int` | `src/pkg/agent/manager.go:264`, `:319` |
| Launch-serialization flag | `launching bool` (guarded by `m.mu`) | `src/pkg/agent/manager.go:274` |
| One-shot bootstrap override | `BootstrapOverride` | `src/pkg/agent/manager.go:276` |
| Token-restart backoff ladder | `lastTokenRestart`, `tokenRestartAttempts`, `tokenRestartGaveUp` | `src/pkg/agent/manager.go:296`, `:285`, `:287` |
| Login / quota observations | `NeedsLogin`, `QuotaExhausted` | `src/pkg/agent/manager.go:308-309` |
| Consent-screen watcher timers | `consentSeenAt`, `lastConsentDismiss` | `src/pkg/agent/manager.go:328-329` |
| Stall-watchdog per-kick state | `lastInferKickAt`, `lastInferKickPane`, `stallNudgeSent`, `lastInferKickMarks`, `actionNudgeSent` | `src/pkg/agent/manager.go:330-333`, `:320`, `:328` |
| Transient-API-error nudge cooldown | `lastTransientNudge`, `transientNudgesThisKick` | `src/pkg/agent/manager.go:337-338` |
| Un-archived-scrollback flag | `kickLogPending` (guarded by `m.mu`) | `src/pkg/agent/manager.go:356` |
| Sandbox / bob-key latches, last launch banner | `sandboxResumeAfterCancel`, `awaitingBobKey`, `lastLaunchFailureBanner` | `src/pkg/agent/manager.go:365`, `:337`, `:346` |
| Poller goroutines themselves | `go m.pollTmuxOutputForAgent(agent, agentCtx)` and siblings, tied to a per-launch context | `src/pkg/agent/manager.go:2955`, `:2571`, `:2577` |
| Blocked-action thrash windows | `Manager.thrash map[string]*thrashState` under its own `thrashMu` — deliberately *not* `m.mu`, to avoid re-entrancy from the output-capture goroutines | `src/pkg/agent/manager.go:477-481`; `thrashState` `:3655`; trip logic `recordBlockedAndCheck` `:3660` |
...
(`src/pkg/agent/manager.go:2173` builds `tmux -L <socket>`). What tmux holds —
...
```
(`src/pkg/agent/manager.go:2938-2967`)
...
- **It is one of two reattach points, and both are early returns rather than a
  ...
  (`src/pkg/agent/manager.go:2569`), whose first act is
  `if m.tmuxSessionExistsForAgent(agent) { return nil }`
  (`src/pkg/agent/manager.go:2570-2572`) — a surviving session is reused, not
  ...
  (`src/pkg/agent/manager.go:2939`).
- **The adoption test is a screen-scrape.** `tmuxPaneHasCLIForAgent`
  (`src/pkg/agent/manager.go:2589`) is one line: `paneHasCLIMarker(
...
`Restart` (`src/pkg/agent/manager.go:10103`) does not resume anything. In order:

1. `C-c` into the pane and cancel the launch context
   (`src/pkg/agent/manager.go:10141-10145`).
2. Archive the scrollback *before* it is destroyed —
   `tearDownTurnLocked(agent, "restart")`
   (`src/pkg/agent/manager.go:10151`), with the comment that `kill-session`
   destroys the only record of the previous run
   (`src/pkg/agent/manager.go:10147-10150`).
3. Reap the CLI process and any UID-owned helpers
   (`src/pkg/agent/manager.go:10159-10163`).
4. `tmux kill-session` (`src/pkg/agent/manager.go:10170`).

The replacement CLI starts with **no prompt at all** in the default case.
`buildBootstrapPrompt` (`src/pkg/agent/manager.go:3882`) unconditionally returns `""`
(`src/pkg/agent/manager.go:3890`), with the reasoning recorded inline: the
...
placeholders to the agent (`src/pkg/agent/manager.go:3883-3888`).
...
directly: `RestartWithBootstrap` (`src/pkg/agent/manager.go:9810`) sets
`BootstrapOverride`, and `RestartThenSendKick`
(`src/pkg/agent/manager.go:9886`) is documented as restarting "with a clean
slate". Whichever path runs, delivery is gated on the new pane reaching its
input marker (`deliverStartupKick`, `src/pkg/agent/manager.go:5471`).
...
command line as a string (`src/pkg/agent/manager.go:10671`, `:2456`, `:2479`),
...
(`src/pkg/agent/manager.go:9413`, helper at `src/pkg/agent/manager.go:7932`) so
...
detection (`src/pkg/agent/manager.go:6245`), auth failure
(`src/pkg/watchdog/classify.go:98`), quota exhaustion
(`src/pkg/agent/manager.go:309`), and even whether a surviving CLI can be
adopted (`src/pkg/agent/manager.go:2589`) are all decided by matching substrings
...
2. **Should the manager-side nudge and restart counters join the persisted
   envelope?** `tokenRestartAttempts` (`src/pkg/agent/manager.go:305`),
   `transientNudgesThisKick` (`src/pkg/agent/manager.go:338`), and
   `stallNudgeSent` (`src/pkg/agent/manager.go:332`) are loop-breakers that
...
4. **Is `buildBootstrapPrompt`'s path-building dead code, or a staging post?**
   *Since answered: dead code.* The function used to assemble a candidate
   ...
   `return ""` stub (`src/pkg/agent/manager.go:3882-3891`, `return ""` at
...
5. **Are there non-tmux agent execution paths with different properties?**
   ...
   (`IsInferenceBackend`, `src/pkg/agent/manager.go:767`). This spike scoped
