...
(`Client.CreateIssueComment`, `src/pkg/github/client.go:1167`).
...
`ValidateChannels` now rejects them (`src/pkg/config/config.go:665`, #5591).
...
(`src/pkg/github/client.go:1167`), gated by `Converse` at the proxy. **Nothing
...
1. **Who may summon — fail closed.** A mention is honoured only from a login
   ...
   (`DashboardConfig.AuthorizedRole`, `src/pkg/config/config.go:4186`) at
   `read-write` or above by default — the same lookup `trustedMergerFunc`
   uses for the merge queue (`src/cmd/hive/main.go:8098`). An explicit
...
- `src/pkg/config/config.go:1685` — `linear.session_agent` resolution rule.
- `src/pkg/config/config.go:665` — the removed declarative channel types (#5591).
...
- `src/pkg/github/client.go:1167` — canary-gated, scrubbed comment posting.
...
- `src/cmd/hive/main.go:8098` — `trustedMergerFunc`, the role-list lookup to
  reuse for summoners.
