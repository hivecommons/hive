...
(`config/backends.conf:69`, mirrored at `src/pkg/agent/manager.go:10671`, base
...
- Hub-pod agents: `launchInTmux` starts the session with `new-session -d -s
  session -c dir` (`src/pkg/agent/manager.go:2245`), then, when the agent has a
  UID assigned, runs the tmux command through `su-exec`/an exec-user spec
  (`agentExecUserSpec`, `src/pkg/agent/manager.go:2257-2265`,
  `src/pkg/agent/manager.go:2286-2291`) so the CLI runs as `hive-<agent>` or a
...
(`src/pkg/agent/manager.go:7788-7794`, shell mirror at
...
`claude_family_perm_flag`) and hub-pod (`src/pkg/agent/manager.go:10675`, `base +
...
either path when set truthy (`src/pkg/agent/manager.go:7810-7815`,
...
anything in `src/pkg/agent/manager.go:7770-7826` or
...
1. **Hub-hosted pod agents (containerized hive deployment).** `src/pkg/agent/manager.go`
   ...
   (`src/pkg/agent/manager.go:2257-2265`) but no filesystem namespace scoping —
...
- **Dedicated low-privilege UID with no polkit/sudo rights, for the
  ...
  `src/pkg/agent/manager.go:2257-2265`), but that map is scoped to hub-pod
