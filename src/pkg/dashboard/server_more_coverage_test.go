...
func TestCovH2_IsPublicPath(t *testing.T) {
	...
	privates := []string{"/api/status", "/api/agents", "/", "/dashboard", "/design-system.html", "/api/config/github", "/api/gh-user-auth/logout", "/tokens.css.map", "/components.css.map", "/api/theme.cssx"}
