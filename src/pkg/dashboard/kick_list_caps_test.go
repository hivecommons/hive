...
func TestGovernorRepos_CapOnlySaveIsAccepted(t *testing.T) {
	...
	if got := srv.deps.Config.Project.MaxPRsPerKick; got != 12 {
		t.Errorf("MaxPRsPerKick = %d, want 12", got)
	}
}
...
func TestGovernorRepos_SavesBothCaps(t *testing.T) {
	...
	if got := srv.deps.Config.Project.MaxIssuesPerKick; got != 40 {
		t.Errorf("MaxIssuesPerKick = %d, want 40", got)
	}
	if got := srv.deps.Config.Project.MaxPRsPerKick; got != 15 {
		t.Errorf("MaxPRsPerKick = %d, want 15", got)
	}
}
...
func TestGovernorRepos_OmittedCapIsUnchanged(t *testing.T) {
	srv := newFullServer(t)
	srv.deps.Config.Project.MaxIssuesPerKick = 55
	srv.deps.Config.Project.MaxPRsPerKick = 22
	...
	if got := srv.deps.Config.Project.MaxIssuesPerKick; got != 55 {
		t.Errorf("MaxIssuesPerKick = %d, want it left at 55", got)
	}
	if got := srv.deps.Config.Project.MaxPRsPerKick; got != 22 {
		t.Errorf("MaxPRsPerKick = %d, want it left at 22", got)
	}
}
...
func TestGovernorRepos_RejectedCapLeavesConfigUntouched(t *testing.T) {
	srv := newFullServer(t)
	srv.deps.Config.Project.MaxPRsPerKick = 25
	...
	if got := srv.deps.Config.Project.MaxIssuesPerKick; got != 0 {
		t.Errorf("MaxIssuesPerKick = %d, want 0 — the valid field must not apply when the request is rejected", got)
	}
	if got := srv.deps.Config.Project.MaxPRsPerKick; got != 25 {
		t.Errorf("MaxPRsPerKick = %d, want it left at 25", got)
	}
}
...
func TestGovernorRepos_ZeroCapIsAcceptedAsUnset(t *testing.T) {
	srv := newFullServer(t)
	srv.deps.Config.Project.MaxPRsPerKick = 12
	...
	if got := srv.deps.Config.Project.MaxPRsPerKick; got != 0 {
		t.Errorf("MaxPRsPerKick = %d, want 0 (unset)", got)
	}
	if got := srv.deps.Config.Project.PRListCap(); got != config.DefaultMaxPRsPerKick {
...
func TestGovernorConfig_ExposesEffectiveCaps(t *testing.T) {
	srv := newFullServer(t)
	srv.deps.Config.Project.MaxIssuesPerKick = 0 // unset
	srv.deps.Config.Project.MaxPRsPerKick = 17
