// Task-prompt rendering for the contributor relay (ClankeR): the pure domain
// that turns public task metadata (repo, issue ref, title, source, base
// branch, access mode) into the exact assignment prompt shipped in a
// task_assign frame. Extracted from contribute_ws.go; every function here is
// a pure renderer or a read-only snapshot — no hub state, no credentials.
// Names on the wire are deliberately unchanged.
package dashboard
