...
func runInjectionExchange(t *testing.T, p *GitHubProxy, agentName string, mode agent.AgentMode, rawReq string) upstreamCapture {
	...
	go func() {
		p.proxyHTTP(proxyClient, proxyUpstream, agentName, mode)
		close(proxyDone)
	}()
...
func TestInjectAuth_GitPathBasicAndConnectionClose(t *testing.T) {
	origDrain := tunnelHalfCloseDrain
	tunnelHalfCloseDrain = 100 * time.Millisecond
	defer func() { tunnelHalfCloseDrain = origDrain }()
