...
// Restart telemetry for pkg/agent — the per-agent restart event ledger that
// backs the dashboard's restart counts, its 24h pruning window and the reason
// sanitiser. Split verbatim out of manager.go (#7303); the restart paths that
// record into it live in manager_restart.go.
