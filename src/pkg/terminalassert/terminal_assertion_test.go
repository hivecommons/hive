...
func TestTerminalAssertionRoundTrip(t *testing.T) {
	...
	tok := Mint(secret, "alice", "owner", "hosted-hive-1", now)
	if tok == "" {
		t.Fatal("expected a token")
	}
	user, role, err := Verify(secret, tok, "hosted-hive-1", now)
	if err != nil {
...
func TestTerminalAssertionRejectsWrongHive(t *testing.T) {
	secret := "s"
	now := time.Unix(1_700_000_000, 0)
	tok := Mint(secret, "alice", "owner", "hive-A", now)
	if _, _, err := Verify(secret, tok, "hive-B", now); err == nil {
...
func TestTerminalAssertionRejectsWrongSecret(t *testing.T) {
	now := time.Unix(1_700_000_000, 0)
	tok := Mint("secret-1", "alice", "owner", "hive-1", now)
	if _, _, err := Verify("secret-2", tok, "hive-1", now); err == nil {
...
func TestTerminalAssertionRejectsExpired(t *testing.T) {
	secret := "s"
	now := time.Unix(1_700_000_000, 0)
	tok := Mint(secret, "alice", "owner", "hive-1", now)
	later := now.Add(terminalAssertionTTL + clockSkew + time.Second)
	if _, _, err := Verify(secret, tok, "hive-1", later); err == nil {
...
func TestTerminalAssertionRejectsNotYetValid(t *testing.T) {
	...
	tok := Mint(secret, "alice", "owner", "hive-1", now)
	earlier := now.Add(-(clockSkew + time.Minute))
	if _, _, err := Verify(secret, tok, "hive-1", earlier); err == nil {
...
func TestTerminalAssertionRejectsTamper(t *testing.T) {
	secret := "s"
	now := time.Unix(1_700_000_000, 0)
	tok := Mint(secret, "alice", "read-write", "hive-1", now)
	b := []byte(tok)
	if b[0] == 'A' {
		b[0] = 'B'
	} else {
	...
	if _, _, err := Verify(secret, string(b), "hive-1", now); err == nil {
...
// (The companion property — a terminal assertion must not verify as an SSO
// handoff token — is exercised in pkg/hub, next to the SSO verifier.)
...
func TestDeriveTerminalKeyIsDomainSeparatedAndPerHive(t *testing.T) {
	...
	got := DerivePerHiveKey(master, InfoKey, "hive-1")
	if got == "" {
...
func TestTerminalSigningKeyResolutionOrder(t *testing.T) {
	...
	if SigningKey() != "dedicated-terminal-key" {
...
func TestN3_TerminalKeyNeverFallsThroughToSessionKey(t *testing.T) {
	...
	if want := DerivePerHiveKey("master", InfoKey, "hive-1"); got != want {
	...
	if strings.Contains(fn, "HIVE_SESSION_KEY") || strings.Contains(fn, "envSessionKey") {
		t.Fatal("N3 REGRESSION: SigningKey's body references the fleet-uniform session key")
	}
	...
	if !strings.Contains(fn, "EnvTerminalKey") || !strings.Contains(fn, "DerivePerHiveKey") {
		t.Fatalf("positive control failed: extracted body is not SigningKey:\n%s", fn)
	}
}

// terminalSigningKeyFuncBody extracts the source text of SigningKey so
...
func terminalSigningKeyFuncBody(t *testing.T, src string) string {
	t.Helper()
	const sig = "func SigningKey() string {"
	i := strings.Index(src, sig)
	if i < 0 {
		t.Fatal("SigningKey not found in source")
	}
	rest := src[i:]
	end := strings.Index(rest, "\n}")
	if end < 0 {
		t.Fatal("could not find end of SigningKey")
	}
	return rest[:end]
}
...
func TestN3_TerminalAssertionDoesNotCrossHives(t *testing.T) {
	...
	keyA := DerivePerHiveKey(master, InfoKey, "hive-a")
	keyB := DerivePerHiveKey(master, InfoKey, "hive-b")
	if keyA == "" || keyB == "" {
...
func forgeTerminalToken(key string, c claims) string {
