...
func TestLoginScanDecision_ValidCredentialNeverPauses(t *testing.T) {
	...
	for _, sightings := range []int{1, 2, 3, 99} {
		action, re := Decision("claude", loginResiduePane, compiled, true, sightings)
		if action != ActionDeferAuthenticated {
	...
	if action, _ := Decision("claude", loginResiduePane, compiled, false, PauseMinSightings); action != ActionPause {
...
func TestLoginScanDecision_StreakDefersTheFirstSighting(t *testing.T) {
	compiled := defaultLoginRegexps(t)
	for s := 1; s < PauseMinSightings; s++ {
		if action, _ := Decision("claude", loginResiduePane, compiled, false, s); action != ActionDeferStreak {
			t.Fatalf("sightings=%d: got %v, want defer-streak", s, action)
		}
	}
	if action, _ := Decision("claude", loginResiduePane, compiled, false, PauseMinSightings); action != ActionPause {
		t.Fatalf("sightings=%d: got %v, want pause", PauseMinSightings, action)
	}
}
...
func TestLoginScanDecision_UnrelatedPaneIsIgnored(t *testing.T) {
	...
	for _, credentialValid := range []bool{true, false} {
		if action, re := Decision("claude", working, compiled, credentialValid, 99); action != ActionIgnore || re != nil {
...
func TestLoginScanDecision_BlockingPromptStandsDown(t *testing.T) {
	...
	if action, _ := Decision("copilot", trustPane, compiled, false, 99); action != ActionIgnore {
...
func TestLoginSightingTracker_NilFallsBackToSingleObservation(t *testing.T) {
	var tr *SightingTracker
	if got := tr.Observe("supervisor", true); got < PauseMinSightings {
...
func TestLoginScanVerdict_NoMatchAlwaysIgnores(t *testing.T) {
	for _, credentialValid := range []bool{true, false} {
		for _, sightings := range []int{0, 1, PauseMinSightings, 99} {
			if got := Verdict(false, credentialValid, sightings); got != ActionIgnore {
...
func TestLoginScanMatch_ReturnsThePatternItMatched(t *testing.T) {
	compiled := defaultLoginRegexps(t)
	re := Match("claude", loginResiduePane, compiled)
	if re == nil {
