...
func TestNewAppAuthFromPEMAndAccessors(t *testing.T) {
	auth, err := NewAppAuthFromPEM(1234, 5678, []byte(testAppPrivateKeyPEM(t)), slog.Default(), "https://github.ibm.com/api/v3")
	if err != nil {
...
func TestNewAppAuthFromPEMRejectsInvalidKey(t *testing.T) {
	if _, err := NewAppAuthFromPEM(1, 2, []byte("not pem"), slog.Default(), ""); err == nil || !strings.Contains(err.Error(), "PEM") {
...
func TestInstallationAccountLoginRejectsInvalidInputs(t *testing.T) {
	...
	auth, err := NewAppAuthFromPEM(1, 0, []byte(testAppPrivateKeyPEM(t)), slog.Default(), "")
	if err != nil {
		t.Fatalf("NewAppAuthFromPEM: %v", err)
	}
...
func TestInstallationAccountLoginReturnsAccount(t *testing.T) {
	...
	defer api.Close()
	auth, err := NewAppAuthFromPEM(1, 0, []byte(testAppPrivateKeyPEM(t)), slog.Default(), api.URL)
	if err != nil {
		t.Fatalf("NewAppAuthFromPEM: %v", err)
	}
