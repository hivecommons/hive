...
func (c *Client) handleOneIssueRequest(ctx context.Context, path string, nowFn func() time.Time) {
	...
	if err != nil {
		c.writeIssueResult(path, IssueResponse{OK: false, Error: "invalid request file: " + err.Error(), At: nowFn().UTC().Format(time.RFC3339)})
	...
	c.recordCreationAudit(action, meta,
		...
		"managed_dedupe", strconv.FormatBool(resp.DeduplicatedAgainstManaged))
	c.writeIssueResult(path, resp)
	_ = os.Remove(path)
	c.issueClearRetry(path)
...
func (c *Client) createOrReuseAgentIssue(ctx context.Context, req IssueRequest, body, marker string) (int, string, bool, bool, error) {
	if c == nil || c.client == nil {
		return 0, "", false, false, ErrNoGitHubClient
	}
	...
	title := logscrub.ScrubString(strings.TrimSpace(req.Title))
	...
	for {
		...
		issues, response, err := c.client.Issues.ListByRepo(ctx, owner, repo, &gh.IssueListByRepoOptions{
			State: "open", Sort: "created", Direction: "desc", ListOptions: gh.ListOptions{Page: page, PerPage: 100},
		})
		if err != nil {
		...
		if response == nil || response.NextPage == 0 || page >= 3 {
			break
		}
		page = response.NextPage
	}
	for _, issue := range open {
		if strings.Contains(issue.GetBody(), marker) || strings.TrimSpace(issue.GetTitle()) == title {
	...
	for _, l := range labels {
		...
		if err := c.ensureLabel(ctx, owner, repo, l); err != nil {
			c.logger.Warn("CreateIssue: could not ensure label, creating without it", slog.String("repo", repo), slog.String("label", l), slog.String("error", err.Error()))
			continue
		}
		usable = append(usable, l)
	}
	created, _, err := c.client.Issues.Create(ctx, owner, repo, &gh.IssueRequest{
		Title: gh.Ptr(title), Body: gh.Ptr(body), Labels: &usable,
	})
	if err != nil {
