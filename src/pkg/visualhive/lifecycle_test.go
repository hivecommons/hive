...
func TestQueueAuthoritativeIssueCloseFailsClosed(t *testing.T) {
	tests := []struct {
		name      string
		mutate    func(*LifecycleStore, string, *VerifiedPacketIdentity)
		wantError string
	}{
		{
			name: "finding is absent",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				delete(store.state.Findings, fingerprint)
			},
			wantError: "is not present",
		},
		{
			name: "packet digest differs",
			mutate: func(_ *LifecycleStore, _ string, packet *VerifiedPacketIdentity) {
				packet.PacketDigest = strings.Repeat("d", 64)
			},
			wantError: "does not match the verified reconciliation packet",
		},
		{
			name: "packet id differs",
			mutate: func(_ *LifecycleStore, _ string, packet *VerifiedPacketIdentity) {
				packet.PacketID = "different-authoritative-packet"
			},
			wantError: "does not match the verified reconciliation packet",
		},
		{
			name: "finding is not resolved",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				store.state.Findings[fingerprint].Status = StatusIssueOpen
			},
			wantError: "not awaiting an authoritative issue close",
		},
		{
			name: "resolved timestamp is absent",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				store.state.Findings[fingerprint].ResolvedAt = nil
			},
			wantError: "not awaiting an authoritative issue close",
		},
		{
			name: "close action is not pending",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				store.state.Findings[fingerprint].PendingIssueAction = OutboxUpdateIssue
			},
			wantError: "not awaiting an authoritative issue close",
		},
		{
			name: "issue number is absent",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				store.state.Findings[fingerprint].IssueNumber = 0
			},
			wantError: "requires the persisted GitHub issue identity",
		},
		{
			name: "issue url is absent",
			mutate: func(store *LifecycleStore, fingerprint string, _ *VerifiedPacketIdentity) {
				store.state.Findings[fingerprint].IssueURL = "  "
			},
			wantError: "requires the persisted GitHub issue identity",
		},
	}
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			store, fingerprint, packet := newAuthoritativeIssueCloseFixture(t)
			store.mu.Lock()
			test.mutate(store, fingerprint, &packet)
			before := mustMarshalLifecycleState(t, store.state)
			store.mu.Unlock()

			queued, err := store.QueueAuthoritativeIssueClose(fingerprint, packet)
			if queued || err == nil || !strings.Contains(err.Error(), test.wantError) {
				t.Fatalf("fail-closed guard returned queued=%v err=%v; want %q", queued, err, test.wantError)
			}
			after := mustMarshalLifecycleState(t, store.Snapshot())
			if !bytes.Equal(before, after) {
				t.Fatalf("denied authoritative close mutated lifecycle state\nbefore=%s\nafter=%s", before, after)
			}
		})
	}
}

func TestQueueAuthoritativeIssueCloseDuplicateIsIdempotent(t *testing.T) {
	store, fingerprint, packet := newAuthoritativeIssueCloseFixture(t)
	store.mu.Lock()
	finding := store.state.Findings[fingerprint]
	manifest := Manifest{
	...
	entry := outboxForFinding(OutboxCloseIssue, finding, manifest, time.Now().UTC())
	store.state.Outbox = append(store.state.Outbox, &entry)
	before := mustMarshalLifecycleState(t, store.state)
	store.mu.Unlock()

	queued, err := store.QueueAuthoritativeIssueClose(fingerprint, packet)
	if err != nil || queued {
		t.Fatalf("duplicate authoritative close was not an idempotent no-op: queued=%v err=%v", queued, err)
	}
	after := mustMarshalLifecycleState(t, store.Snapshot())
	if !bytes.Equal(before, after) {
		t.Fatalf("duplicate authoritative close mutated lifecycle state\nbefore=%s\nafter=%s", before, after)
	}
}

func TestQueueAuthoritativeIssueCloseRollsBackOnAuditFailure(t *testing.T) {
	store, fingerprint, packet := newAuthoritativeIssueCloseFixture(t)
	before := mustMarshalLifecycleState(t, store.Snapshot())
	if err := os.Mkdir(store.auditPath, 0o700); err != nil {
		t.Fatal(err)
	}

	queued, err := store.QueueAuthoritativeIssueClose(fingerprint, packet)
	if queued || err == nil || !strings.Contains(err.Error(), "audit path") {
		t.Fatalf("audit failure did not reject authoritative close: queued=%v err=%v", queued, err)
	}
	after := mustMarshalLifecycleState(t, store.Snapshot())
	if !bytes.Equal(before, after) || len(store.PendingOutbox()) != 0 {
		t.Fatalf("audit failure did not roll back authoritative close\nbefore=%s\nafter=%s", before, after)
	}
	...
	queued, err = store.QueueAuthoritativeIssueClose(fingerprint, packet)
	if err != nil || !queued || len(store.PendingOutbox()) != 1 {
		t.Fatalf("authoritative close did not recover after audit repair: queued=%v err=%v pending=%+v", queued, err, store.PendingOutbox())
	}
}

func TestQueueAuthoritativeIssueCloseRollsBackOnPersistenceFailure(t *testing.T) {
	store, fingerprint, packet := newAuthoritativeIssueCloseFixture(t)
	store.mu.Lock()
	...
	stateBackup := store.statePath + ".backup"
	if err := os.Rename(store.statePath, stateBackup); err != nil {
		t.Fatal(err)
	}
	...
	queued, err := store.QueueAuthoritativeIssueClose(fingerprint, packet)
	if queued || err == nil || !strings.Contains(err.Error(), "publish lifecycle state") {
		t.Fatalf("state publication failure did not reject authoritative close: queued=%v err=%v", queued, err)
	}
	after := mustMarshalLifecycleState(t, store.Snapshot())
	if !bytes.Equal(before, after) || len(store.PendingOutbox()) != 0 {
		t.Fatalf("state publication failure did not roll back authoritative close\nbefore=%s\nafter=%s", before, after)
	}

	if err := os.Remove(store.statePath); err != nil {
		t.Fatal(err)
	}
	if err := os.Rename(stateBackup, store.statePath); err != nil {
		t.Fatal(err)
	}
	queued, err = store.QueueAuthoritativeIssueClose(fingerprint, packet)
	if err != nil || !queued || len(store.PendingOutbox()) != 1 {
		t.Fatalf("authoritative close did not recover after state repair: queued=%v err=%v pending=%+v", queued, err, store.PendingOutbox())
	}
	if count := countLifecycleAuditAction(t, store.auditPath, "queue_authoritative_issue_close"); count != 1 {
		t.Fatalf("audit-before-state retry appended %d authoritative-close receipts; want exactly one", count)
	}
}

func newAuthoritativeIssueCloseFixture(t *testing.T) (*LifecycleStore, string, VerifiedPacketIdentity) {
	t.Helper()
	store, err := NewLifecycleStore(filepath.Join(t.TempDir(), "lifecycle"))
	if err != nil {
		t.Fatal(err)
	}
	fingerprint := strings.Repeat("a", 64)
	packet := VerifiedPacketIdentity{
		PacketID: "packet-authoritative-close", PacketDigest: strings.Repeat("b", 64),
		BaseSHA: strings.Repeat("c", 40), WorkflowRunID: "12345",
	}
	resolvedAt := time.Unix(100, 0).UTC()
	store.mu.Lock()
	store.state.Findings[fingerprint] = &FindingLifecycle{
		Repository: "owner/repo", RepositoryID: "123", Fingerprint: "source-fingerprint",
		RepositoryFingerprint: fingerprint, Status: StatusResolved, ResolvedAt: &resolvedAt,
		IssueNumber: 77, IssueURL: "https://github.test/owner/repo/issues/77",
		PendingIssueAction: OutboxCloseIssue, LastBundleID: packet.PacketID,
		LastBundleDigest: packet.PacketDigest, Title: "Resolved finding", Body: "Evidence-bound finding",
	}
	store.mu.Unlock()
	return store, fingerprint, packet
}

func mustMarshalLifecycleState(t *testing.T, state LifecycleState) []byte {
	t.Helper()
