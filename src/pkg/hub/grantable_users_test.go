...
func TestManageAccessDialogHasUserSearch(t *testing.T) {
	for _, want := range []string{
		...
		`id="access-add-btn"`,
		`id="access-user-toggle"`,
		`onclick="toggleAccessUserResults()"`,
		`oninput="filterAccessUserDropdown()"`,
		`onkeydown="accessUserSearchKeydown(event)"`,
		`onfocus="openAccessUserResults()"`,
		`function filterAccessUserDropdown()`,
		`var ACCESS_USER_DEBOUNCE_MS = 200;`,
		`var ACCESS_USER_RESULT_LIMIT = 200;`,
		`function toggleAccessUserResults()`,
		`function renderAccessUserOptions(filter)`,
		`}).sort(function(a, b)`,
		`.slice(0, ACCESS_USER_RESULT_LIMIT)`,
		`type to filter`,
		`type to narrow`,
		`max-height: 300px`,
