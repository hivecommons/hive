...
// mintHubUserCookieValueV2 lives in hub_cookie_legacy_test.go: production mints
// only v3 (oauth.go), so the v2 mint lane is retained solely for the regression
// tests that prove v2 cookies still verify during the rollout window.
...
// mintHubUserCookieValue, verifyHubUserCookieValue, and hubCookieSign — the
// legacy symmetric-HMAC cookie lane — live in hub_cookie_legacy_test.go. The
// verify side of that lane was deleted from verifyHubUserCookieEitherAt (AUDIT
// F1), so the mint machinery is retained only as a forgery primitive for the
// regression tests that prove legacy cookies are rejected. Keeping it out of
// this file keeps it out of every shipped binary.

// verifyHubUserCookieEither — the 3-argument convenience wrapper over
// verifyHubUserCookieEitherAt — lives in hub_cookie_legacy_test.go: every
// production call site uses verifyHubUserCookieEitherAt directly (via
// HubServer.verifyHubUserCookie or the generation-aware verifier), so the
// wrapper survives only for the tests written against its original shape.

// verifyHubUserCookieEitherAt verifies a session cookie with the verifier's
...
// hubCookieIsV3 / hubCookieIsV2 (rollout-telemetry format probes) live in
// hub_cookie_legacy_test.go: they were never wired to any production caller.
