...
// restoreGlobalProvider resets the global TracerProvider to a no-op provider
// on cleanup, so a test that installs a recording provider (directly or via
// Init) does not leak it into other tests under -shuffle=on. Snapshot-and-
// restore of the previous provider is NOT possible here: before any test has
// replaced it, otel.GetTracerProvider returns the package's internal delegator,
// which otel.SetTracerProvider refuses to reinstall — the "restore" silently
// no-ops and the recording provider stays global (the #5895 flake).
