...
#   pct exec <CTID> -- bash -c 'curl -fsSL https://raw.githubusercontent.com/kubestellar/hive/v2/src/deploy/bootstrap-lxc.sh | bash'
...
echo "  pct exec ${CTID} -- bash -c 'apt-get update -qq && apt-get install -y -qq curl && curl -fsSL https://raw.githubusercontent.com/kubestellar/hive/v2/src/deploy/bootstrap-lxc.sh | bash'"
echo ""
echo "Or via SSH:"
if [[ "${KEY_ONLY}" -eq 1 ]]; then
  echo "  ssh root@${LXC_IP} 'apt-get update -qq && apt-get install -y -qq curl && curl -fsSL https://raw.githubusercontent.com/kubestellar/hive/v2/src/deploy/bootstrap-lxc.sh | bash'"
else
  echo "  sshpass -p ${PASSWORD} ssh root@${LXC_IP} 'apt-get update -qq && apt-get install -y -qq curl && curl -fsSL https://raw.githubusercontent.com/kubestellar/hive/v2/src/deploy/bootstrap-lxc.sh | bash'"
fi
