...
- `server.js` and `index.html` — legacy Node dashboard assets from the pre-v2 dashboard path. They are useful historical/reference material, but v2 production does not start `dashboard/server.js`. The legacy server binds `127.0.0.1` by default because many historical control endpoints are unauthenticated; the Nous mutation endpoints require the owner/operator dashboard bearer token or proof-verified owner proxy headers. Set `HIVE_DASHBOARD_BIND=0.0.0.0` only behind an authenticated proxy.
