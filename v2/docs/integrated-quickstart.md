...
Hosted dashboards keep GitHub credentials out of agent terminals. An authenticated owner can instead use the dashboard control-plane endpoints `POST /api/integrated/setup/plan` and `POST /api/integrated/setup/apply`. Both derive the repository from the running Hive configuration, require the saved GitHub device-flow identity to match the active dashboard owner, and invoke the same `hive setup` implementation with `--runtime local`. The plan response returns `plan_sha256`; apply requires that exact digest as `expected_plan_sha256` and re-runs the plan before any mutation. The credential remains in the dashboard process and is never returned, logged, or added to an agent environment. These endpoints do not start a second scheduler; restart the ordinary Hive/dashboard after the managed setup PR merges.
