...
func TestForceRegister_ExistingContributor_Unauthenticated_401(t *testing.T) {
	// MERGE-REVIEW (v2→v4 sync): this v2-only test asserts v2's #2610
	// gated-force-rotation behavior, but the sync KEPT v4's stricter source
	// (force:true ignored on register — no reissue at all). Against v4's code
	// this expects 401 where v4 returns 200, so it is skipped pending a
	// maintainer decision: adopt v2's gated-force source, or delete this file.
	t.Skip("MERGE-REVIEW: v4 kept the force-ignored register posture; see api_contribute.go MERGE-REVIEW")
...
func TestForceRegister_ExistingContributor_WrongOwner_403(t *testing.T) {
	// MERGE-REVIEW (v2→v4 sync): see file-top note. v4 returns 200 (force
	// ignored) where this v2 test expects 403; skipped pending maintainer review.
	t.Skip("MERGE-REVIEW: v4 kept the force-ignored register posture; see api_contribute.go MERGE-REVIEW")
	setupContributeEnv(t)
	s := NewServer(0, slog.Default())
	s.registerContributeRoutes()
...
func TestForceRegister_ExistingContributor_Owner_Succeeds(t *testing.T) {
	// MERGE-REVIEW (v2→v4 sync): see file-top note. v4 returns 200 with NO new
	// token (force ignored) where this v2 test expects a reissued token; skipped
