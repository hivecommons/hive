...
func knowledgeWriteCommand(env *commandEnv, operation string) *cobra.Command {
	...
	command := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if operation == "update" && len(args) != 2 {
				return &usageError{message: "knowledge update requires <layer> <slug>"}
			}
			if operation == "create" && len(args) != 0 {
				return &usageError{message: "knowledge create does not accept positional arguments"}
			}
