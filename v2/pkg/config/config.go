...
func (c *Config) persistencePayload() ([]byte, error) {
	...
	// Redact env-sourced secrets first (v4 security invariant), then restore
	// load-proven env templates so frozen payload values persist verbatim.
	data, err := c.redactedForPersist().persistenceBytes()
	if err != nil {
