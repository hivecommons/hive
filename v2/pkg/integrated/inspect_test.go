...
func TestPullRequestWorkflowIsReadOnlyPinnedAndVerdictEnforcing(t *testing.T) {
	...
	for _, required := range []string{checkoutActionSHA, setupNodeActionSHA, setupPythonActionSHA, uploadArtifactActionSHA, downloadArtifactActionSHA, "name: Hive repository test 001", "node --test", "visual-hive-execution", "visual-hive-pr-raw-${{ github.run_id }}-${{ github.run_attempt }}", "name: visual-hive-pr", "pipeline-exit-code.txt", "Repository test plan failed", "Enforce deterministic verdict", "baselines list", "HIVE_NEEDS_JSON: ${{ toJSON(needs) }}", "HIVE_EXPECTED_REPOSITORY_JOBS", "sudo chown -R root:root .git", "Raw evidence contains a symbolic link",
