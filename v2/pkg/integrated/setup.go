...
func setupRequiredActions(options SetupOptions) []string {
	...
	if UsesNormalHiveRuntime(ownerConfig) {
		actions = append(actions,
			...
			"Confirm the existing dashboard listener is HTTP-ready; after setup succeeds, the running dashboard automatically reconciles and activates the installed contract without a restart. Do not use hive run or hive start for this runtime, and use hive stop only when status or doctor directs stale legacy-scheduler cleanup",
		)
