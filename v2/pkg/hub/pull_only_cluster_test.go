...
// pull-only pool would then execute against hive-oke and report success for
// work that never happened there — the silent-wrong-cluster failure, which is
...
// the hub never pays the dial timeout that made 15 vllm-d hives serialise into
// ~30 minutes of blocking.
