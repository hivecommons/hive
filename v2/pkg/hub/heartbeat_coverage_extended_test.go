...
func TestReportUpgradeFailure(t *testing.T) {
	...
	t.Run("sends correct payload to hub", func(t *testing.T) {
		...
		if authHeader != "Bearer test-secret-123" {
			t.Errorf("Authorization = %q, want 'Bearer test-secret-123'", authHeader)
		}
	})
...
func TestStartTaskStatusPushExtended(t *testing.T) {
	...
	t.Run("sets authorization header when secret present", func(t *testing.T) {
		...
		if auth, ok := gotAuth.Load().(string); !ok || auth != "Bearer task-secret" {
			t.Errorf("Authorization = %q, want 'Bearer task-secret'", auth)
		}
	})
}
