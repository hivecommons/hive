...
| `just contribute-tui` | Opens the `hivectl tui` dashboard; press `Shift+H` for the Hives overlay. If this checkout has no `hivectl`, the recipe stages `./bin/hivectl` from the Hive image and records the source digest so later runs refresh or refuse instead of using a stale binary. |
