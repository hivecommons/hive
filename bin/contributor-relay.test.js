...
// ---------------------------------------------------------------------------
// hivecommons/hive#5685 — "this client sent bytes" is not "a human typed".
//
...
// ---------------------------------------------------------------------------
// hivecommons/hive#5715 — the PR review cycle is aborted by its own progress
...
test('#5715 reconnecting mid-review does not re-assert the task', () => {
  ...
  // hivecommons/hive#0 — resuming", then a revoke of that same task.
  const relay = loadRelay({ backend: 'claude' });
  try {
    relay.setCliReady(true);
    const task = reviewTask();
