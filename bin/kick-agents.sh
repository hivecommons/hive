...
_GH_AUTH_INSTR="⚙ GH AUTH: gh is already authenticated — just run gh commands directly (e.g. gh issue list, gh pr view); use hive-open-pr, not raw gh pr create, when opening PRs. Do NOT read, cat, echo, or set GH_TOKEN yourself; do NOT hardcode a token or use a PAT. If a gh command fails with an auth error, report it — do not try to work around it by fetching a token."
...
⛔ MERGE METHOD: ALWAYS use 'gh pr merge <number> --squash --repo <repo>' (or 'hive-merge' where installed) for ALL repos (console, docs, console-kb, etc.). Do NOT rely on Tide or Prow to merge — Prow jobs may not trigger for bot-authored PRs. ⛔ NEVER pass --admin: bypassing branch protection caused the 2026-06-12 merge-loop incident (19 build breaks in 12h — see console .github/SCANNER-GUARDRAILS.md). If GitHub refuses the merge because required checks are pending or failing, WAIT for the next kick. Never use /lgtm or /approve as a merge strategy.
WORKFLOW: Dispatch a sub-agent for each issue (use the Agent tool). Each agent does: 1) git worktree add /tmp/fix-NNNN -b fix/NNNN from the repo checkout, 2) cd into worktree, read the issue with gh issue view, make the fix, 3) git add + git commit -s, 4) run src/scripts/issue-coauthor.sh --amend NNNN if the PR will use 'Fixes #NNNN' (exit 0 with empty output means no human to credit; if resolution fails, warn and continue), 5) git push origin fix/NNNN, 6) run hive-open-pr --body 'Fixes #NNNN'. hive-open-pr checks the trailer seam but never force-pushes; issue-coauthor.sh is the single source of truth for bot/self skips and must NEVER add Signed-off-by for anyone else. Dispatch 4-6 agents IN PARALLEL — do not work issues one-at-a-time. Use the model_recommendation for each issue (S=haiku, M=sonnet, C=opus). Bundle clustered issues into 1 agent per cluster. Skip issues with lane!=scanner. Do NOT stand by — if issues exist, work them. NEVER run vitest, npm test, npm run build, tsc, or any test/build locally. Beads: ~/scanner-beads"
