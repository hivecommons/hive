...
function classifyReadiness(text, backend) {
  if (backend === 'claude') {
      ...
      const recent = paneTail(text, 15);
      // kubestellar/hive#6639 tried to fix a rotating-splash-tip false
      // positive ("Tip: Log in to several accounts of the same provider…")
      // by stripping the "Tip:" paragraph out of the 15-line window before
      // testing onboarding/login. That stripping loop had no way to end: it
      // dropped every line from the "Tip:" line onward until a BLANK line,
      // but the very next line is always a "─"-filled rule (never blank),
      // so on every real capture it silently swallowed the rest of the
      // window — including the actual prompt and response — hiding a real
      // login/onboarding state behind a false 'ready' instead of a false
      // 'needs-login'. Worse, a live Hive OMP contributor (2026-09-11, after
      // 30 completed tasks) hit the ORIGINAL false positive by a different
      // route the tip fix never touched: the agent's own reply in a prior
      // turn plainly discussed authentication ("Authentication required:
      // please sign in to continue" — reproduced live in a disposable
      // session asking OMP to write exactly that sentence), and that turn's
      // prose was still inside the 15-line tail when OMP relaunched,
      // tripping needs-login on a pane that was fully idle and ready.
      //
      // Both are the same root cause: the CLI's real login/onboarding
      // prompts render at the point of interaction — the last couple of
      // non-blank lines, right where the input box sits — never buried
      // under a splash tip or a completed turn's own output. Narrowing the
      // window to the last 3 non-blank lines (checked directly: excludes
      // both captured false positives, still contains the whole two-line
      // omp_login/omp_onboarding fixtures) fixes both without needing to
